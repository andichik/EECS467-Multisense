...
public func perform(_ operation: CompressionOperation,
                    on input: Data,
    ...
    // NOTE: I think this creates a second stream on the stack
...
extension Data {
    ...
    public func compressed(with algorithm: CompressionAlgorithm) -> Data? {
        return perform(.compression, on: self, using: algorithm)
    }
    
    public func decompressed(with algorithm: CompressionAlgorithm) -> Data? {
        return perform(.decompression, on: self, using: algorithm)
    }
}
